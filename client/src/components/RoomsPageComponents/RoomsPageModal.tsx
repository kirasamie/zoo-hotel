...
export default function RoomsPageModal({
  ...
  const changeHandler = (event: SelectChangeEvent) => {
    ...
    if (event.target.name === "description")
      setMainOrder((prev) => ({ ...prev, description: event.target.value }));
  };

  const changeDateRangeHandler = (value: DateRange<unknown>) => {
    const datesString = value.map(
      (date) => date?.$d && new Date(date.$d).toDateString()
    );
    if (datesString.every((el) => el !== undefined)) {
      ...
      const diffDays: number =
        Math.ceil(Math.abs(date2 - date1) / (1000 * 60 * 60 * 24)) + 1;
      setDays(diffDays);
      setMainOrder((prev) => ({
        ...prev,
        userId: user.id,
  ...
  const paymentHandler = async () => {
    ...
    const session = await axios.post(
      `${import.meta.env.VITE_URL}/stripe`,
      { mainOrder: mainOrder, additionalOrders: additionalOrders },
  ...
  const getBannedDates = (dates: [string, string][]) => {
    return dates.map((range) => {
      ...
      while (dateFrom.getTime() <= dateTo.getTime()) {
        result.push(
          pad(dateFrom.getDate()) +
            "." +
            pad(dateFrom.getMonth() + 1) +
            "." +
  ...
  useEffect(() => {
    setOptionsPrice(() =>
      Object.entries(optionsInputs).reduce(
        (acc, el) => (acc += el[1] ? optionsPrices[el[0]] : 0),
        0
      )
    );
  }, [optionsInputs]);
          ...
          <div className={styles.dividedForm}>
            <FormControl error={!!petSelectError} sx={{ flex: 1 }}>
              ...
              >
                {pets.length ? (
                  pets.map((pet) => (
                    <MenuItem value={String(pet.id)}>{pet.petName}</MenuItem>
                  ))
                ) : (
                  <MenuItem>
                    <Button onClick={() => void navigate("/account/pets/new")}>
                      Создайте карточку питомца!
                    </Button>
                  </MenuItem>
                )}
              </Select>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="ru">
              <DateRangePicker
                ...
                shouldDisableDate={(date) => {
                  return getBannedDates(dates)
                    .flat()
                    .includes(new Date(date.$d).toLocaleDateString());
                }}
              ...
              {!!days && (
                <>
                  <ul className={styles.optionsPriceList}>
                    {optionsInputs["1"] && (
                      <li>Груминг: {optionsPrices["1"]} руб</li>
                    )}
                    {optionsInputs["2"] && (
                      <li>Занятия с кинологом: {optionsPrices["2"]} руб</li>
                    )}
                    {optionsInputs["3"] && (
                      <li>
                        Консультация зоопсихолога: {optionsPrices["3"]} руб
                      </li>
                    )}
                    {optionsInputs["4"] && (
                      <li>Зоотакси: {optionsPrices["4"]} руб</li>
                    )}
                    {optionsInputs["5"] && (
                      <li>
                        Приготовление пищи для питомца: {optionsPrices["5"]} руб
                      </li>
                    )}
                    {optionsInputs["6"] && (
                      <li>
                        Фотоотчет более 1 раза в день: {optionsPrices["6"]} руб
                      </li>
                    )}
                    {optionsInputs["7"] && (
                      <li>
                        Подготовка собаки к выставке: {optionsPrices["7"]} руб
                      </li>
                    )}
                    <br />
                    <li>
                      Проживание: {room?.roomPrice} руб за {days} дней ={" "}
                      {room?.roomPrice * days} руб
                    </li>
                  </ul>
                  <span className={styles.finalCost}>
                    Итого к оплате: {room?.roomPrice * days + optionsPrice} руб
                  </span>
                </>
              )}
            </div>
