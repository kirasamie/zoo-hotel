...
export default function RoomsPageModal({ room, open, handleClose }: ModalPropsType) {
  ...
  const [mainOrder, setMainOrder] = useState<OrderType>({ userId: 0, petId: 0, roomId: 0, dateFrom: '', dateTo: '', amount: 0, quantity: 0, description: '', addServices: '' });
  ...
  const [optionsInputs, setOptionInputs] = useState({ '1': false, '2': false, '3': false, '4': false, '5': false, '6': false, '7': false });
  ...
  const optionsChangeHandler = (event) => {
    setOptionInputs((prev) => ({ ...prev, [event.target.name]: event.target.checked }));
  };
